	...
	if (!empty($_POST['productname' . $x]))  {
		...
		switch ($thisProduceType) {
			...
			case "v": //veg
				...
				$produceItem = new veggie($x, $thisProductName, $thisProductPriceType, $thisProductPrice); //removed the underscore of the variable $thisProductName
				$typeType = "Veggie";
				break;
		}
		
		if($produceItem->price_type == 2) {
			$pType = "Each"; //added semicolon to end of statement.
		}
		else {
			$pType = "Per Lb";
		}
